...
export async function getFoodDrinkArticlesWithFallback(): Promise<Article[]> {
  console.log('🔄 Loading Food & Drink articles from Supabase with fallback...')

  try {
    // Try to get all articles from Supabase first (with timeout)
    const allArticles = await getArticlesWithFallback(8000) // 8 second timeout
    console.log(`✅ Loaded ${allArticles.length} articles from Supabase`)
    ...
    console.log(`✅ FOOD & DRINK PAGE: Returning ${foodDrinkArticles.length} food/drink articles (from ${allArticles.length} total)`)
    return foodDrinkArticles // Return all articles for the dedicated page
  } catch (error) {
    console.error('❌ Failed to load Food & Drink articles:', error)
    return []
  }
}
