...
import { getGrandePrairieArticles } from '@/lib/alberta-cities'
import { Article } from "@/lib/types/article"
import { Metadata } from 'next'

export const metadata: Metadata = {
    title: 'Grande Prairie - Culture Alberta',
    description: "Discover the latest news, events, and stories from Grande Prairie. Explore the Peace Region's vibrant culture, attractions, and gateway to northern Alberta.",
    openGraph: {
        title: 'Grande Prairie - Culture Alberta',
        description: "Discover the latest news, events, and stories from Grande Prairie.",
        url: 'https://www.culturealberta.com/grande-prairie',
    },
}
...
async function getGrandePrairieData() {
    try {
        const allContent = await getGrandePrairieArticles() as GrandePrairieArticle[]
            ...
            <main className="flex-1">
                    ...
                    <div className="container mx-auto px-4 md:px-6">
                        <div className="flex flex-col items-center justify-center space-y-2 text-center">
                            <div className="space-y-1">
                                <h1 className="text-4xl font-bold tracking-tighter sm:text-5xl">Grande Prairie</h1>
                            ...
                            <Link href={getArticleUrl(featuredArticle)} className="group block">
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-center">
                                    ...
                                    <div className="space-y-4">
                                        <div className="inline-block px-3 py-1 bg-amber-100 text-amber-800 rounded-full text-sm font-medium">Featured</div>
                                        <h2 className="text-3xl font-bold tracking-tight group-hover:text-amber-700 transition-colors">
                                        ...
                                        <div className="flex items-center gap-4 text-sm text-muted-foreground">
                                            ...
                                            <span className="flex items-center gap-1"><MapPin className="h-4 w-4" />Grande Prairie</span>
                                        </div>
                                    </div>
                                </div>
                            </Link>
                ...
                {trendingArticles.length > 0 && (
                    <section className="w-full py-6 bg-muted/40">
                        <div className="container mx-auto px-4 md:px-6">
                            <h2 className="text-2xl font-bold mb-6">Trending in Grande Prairie</h2>
                                ...
                                {trendingArticles.map((article) => (
                                    <Link key={article.id} href={getArticleUrl(article)} className="group">
                                        <div className="space-y-3">
                                            ...
                                            <h3 className="font-semibold line-clamp-2 group-hover:text-amber-700 transition-colors">{article.title}</h3>
