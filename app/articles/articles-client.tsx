...
function ArticlesContent({ initialArticles }: { initialArticles: Article[] }) {
  const searchParams = useSearchParams()
  const router = useRouter()
  const initialSearch = searchParams.get("search") || ""

  // Seeded from the server so the first render already contains the list.
  //
  // This page used to start empty and fetch /api/articles from useEffect, which
  // runs only in the browser. A client component is still server-rendered, so
  // the shell was there — with nothing in it. Bing crawled /articles, got zero
  // words and zero links, and filed it as a Soft 404. /guides 308s here, so
  // that redirect was landing on a page search engines considered empty too.
  const [allArticles, setAllArticles] = useState<Article[]>(initialArticles)
  const [results, setResults] = useState<Article[]>(initialSearch ? [] : initialArticles)
  const [inputValue, setInputValue] = useState(initialSearch)
  const [activeQuery, setActiveQuery] = useState(initialSearch)
  // Only "loading" when the server handed us nothing to show.
  const [isLoading, setIsLoading] = useState(initialArticles.length === 0)
  const [isSearching, setIsSearching] = useState(false)

  // Re-fetch on mount only if the server pass came back empty, so a failed
  // server read still resolves in the browser rather than showing an empty page.
  useEffect(() => {
