...
import { getContentQualityLabel, getContentQualityScore, validateArticleContent } from "@/lib/content-validation"
import {
  Article
} from "@/lib/data"
import { AlertCircle, CheckCircle, Edit, Loader2, Plus, RefreshCw, Trash2 } from "lucide-react"
...
export default function AdminArticles() {
  ...
  const loadArticles = async (page: number = currentPage, forceRefresh: boolean = false) => {
    setIsLoading(true)
    try {
      ...
      if (data.articles && data.pagination) {
        ...
        if (data.articles.length > 0) {
          ...
          const uniqueLocations = Array.from(new Set(
            data.articles.map((a: any) => a.location).filter((l: string) => l && l.trim() !== '')
          )) as string[]
        ...
        <div className="flex justify-center mt-6">
          <Pagination>
            <PaginationContent>
              <PaginationItem>
                <PaginationPrevious
                  onClick={() => pagination.hasPrevPage && setCurrentPage(p => Math.max(1, p - 1))}
              ...
              <PaginationItem>
                <PaginationNext
                  onClick={() => pagination.hasNextPage && setCurrentPage(p => Math.min(pagination.totalPages, p + 1))}
