...
import { getRedDeerArticles } from '@/lib/alberta-cities'
import { Article } from "@/lib/types/article"
import { Metadata } from 'next'

export const metadata: Metadata = {
    title: 'Red Deer - Culture Alberta',
    description: "Discover the latest news, events, and stories from Red Deer. Explore Central Alberta's vibrant culture, attractions, and local happenings.",
    openGraph: {
        title: 'Red Deer - Culture Alberta',
        description: "Discover the latest news, events, and stories from Red Deer.",
        url: 'https://www.culturealberta.com/red-deer',
    },
}
...
interface RedDeerArticle extends Article {
    type?: string
}

async function getRedDeerData() {
    try {
        console.log('🔄 Loading Red Deer articles...')

        const allContent = await getRedDeerArticles() as RedDeerArticle[]
        const articles = allContent.filter(item => item.type !== 'event')

        console.log(`✅ Red Deer articles loaded: ${articles.length}`)
        ...
        console.error('❌ Error loading Red Deer data:', error)
        return {
            articles: [],
...
export default async function RedDeerPage() {
    const { articles, featuredArticle, trendingArticles } = await getRedDeerData()
    ...
    const outdoorsArticles = articles.filter(a =>
        ...
        a.category?.toLowerCase().includes('nature') ||
            ...
            <main className="flex-1">
                    ...
                    <div className="container mx-auto px-4 md:px-6">
                        <div className="flex flex-col items-center justify-center space-y-2 text-center">
                            <div className="space-y-1">
                                <h1 className="text-4xl font-bold tracking-tighter sm:text-5xl">Red Deer</h1>
                                <p className="max-w-[900px] text-muted-foreground md:text-xl mx-auto">
                                    Discover the latest news, events, and stories from Central Alberta.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                {trendingArticles.length > 0 && (
                    <section className="w-full py-6 bg-muted/40">
                        <div className="container mx-auto px-4 md:px-6">
                            <h2 className="text-2xl font-bold mb-6">Trending in Red Deer</h2>
                    ...
                    <div className="container mx-auto px-4 md:px-6">
                        <Tabs defaultValue="all" className="w-full">
                            ...
                            <TabsContent value="food" className="space-y-6">
                                ...
                                {foodDrinkArticles.length === 0 && (
                                    <p className="text-center text-muted-foreground py-12">
                                        No Food & Drink articles yet. Check back soon!
                                    </p>
                                )}
                            </TabsContent>
                            ...
                            <TabsContent value="culture" className="space-y-6">
                                ...
                                {cultureArticles.length === 0 && (
                                    <p className="text-center text-muted-foreground py-12">
                                        No Arts & Culture articles yet. Check back soon!
                                    </p>
                                )}
                            </TabsContent>
                            ...
                            <TabsContent value="outdoors" className="space-y-6">
                                ...
                                {outdoorsArticles.length === 0 && (
                                    <p className="text-center text-muted-foreground py-12">
                                        No Outdoors articles yet. Check back soon!
                                    </p>
                                )}
                            </TabsContent>
                        </Tabs>
